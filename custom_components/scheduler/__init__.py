...
class SchedulerCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass, session, entry, store):
        ...
        #entity2 = self.hass.data
       # test = is_on(self.hass, entity2)
        #_LOGGER.error(entity2)

        if hass.state == CoreState.running:
            handle_startup(None)
        else:
            self.hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STARTED, handle_startup)   
    ...
    async def async_edit_schedule(self, schedule_id: str, data: dict):
        ...
        #self.hass.states.bus.listen()
        #listen_state(self.change_light,"sensor.node_f0", attribute = "solar")
        #_LOGGER.error(self.hass.states.get("input_boolean.manual_control_corridor_thermostat"))
       # _LOGGER.error(ATTR_NAME)
